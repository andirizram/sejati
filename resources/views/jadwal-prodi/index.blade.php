...
@section('content')
    ...
    <div class="container">
            ...
            <div>
                @can('jadwal-prodi.store')
                    <button type="button" class="btn btn-primary" id="btn-create-modal"
                            data-action="{{ route('jadwal-prodi.store') }}"
        ...
        <div class="panel panel-default">
            ...
            <p>Jadwal akademik untuk PRODI Teknik Informatika ITERA.</p>
...
@section('script')
    ...
    <script type="module">
        $(document).ready(function () {
            ...
            @error('edit')
            const action = '{{ route('jadwal-prodi.update', $errors->first('id'))}}';
            $('#modal').find('button[data-action="' + action + '"]').trigger('click');
            @enderror

            $('#modal').on('show.bs.modal', function () {
                //     put data type category to form from global data jadwal
                if (jadwal == null) {
                    return;
                }
                ...
                $('#modal').find('input[name="kelas"]').val(detailJadwal.kelas);
