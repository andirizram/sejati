var vanillaSlideshow = (function () {
    ...
    var slides = slideshow
        ? slideshow.getElementsByClassName("vanilla-slide")
        : [];
    ...
    function _createIndicators() {
        var indicatorsElem = document.getElementById(indicatorsContainer);
        if (!indicatorsElem) {
            console.error(
                'Element with id "' + indicatorsContainer + '" not found'
            );
            return;
        }
        ...
        for (var i = 0; i < slides.length; i++) {
            ...
            var indicators = indicatorsElem.appendChild(node);
        ...
        app.indicators =
            slideshowContainer.getElementsByClassName(indicatorsContainer);
    }
    ...
    function _indicatorsClick(self) {
        ...
        for (var i = 0; i < slides.length; i++) {
            if (_hasClass(app.indicators[i], "vanilla-active")) {
                app.indicators[i].className = app.indicators[
                    i
                ].className.replace(/(?:^|\s)vanilla-active(?!\S)/g, "");
            }
            if (_hasClass(slides[i], "vanilla-active")) {
    ...
    app.init = function (options) {
        ...
        if (options && typeof options === "object") {
            defaults.arrows =
                options.arrows !== "" ? options.arrows : defaults.arrows;
            defaults.indicators =
                options.indicators !== ""
                    ? options.indicators
                    : defaults.indicators;
            defaults.random =
                options.random !== "" ? options.random : defaults.random;
            defaults.slideshow =
                options.slideshow !== ""
                    ? options.slideshow
                    : defaults.slideshow;
            defaults.delay = options.delay ? options.delay : defaults.delay;
            defaults.animationSpeed = options.animationSpeed
...
export default vanillaSlideshow;
